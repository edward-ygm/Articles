<template>
    <div class="btn-group" :style="getDropStyle">
        <button type="button" class="btn btn-light " data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"  :style="getButtonStyle">
          <SvgIcons iconID="moreFunc"  :iconSize="siHeight"></SvgIcons>
        </button>
        <div class="dropdown-menu pull-left" :style="getdropdownStyle">
            <li>
                <div class="dropdown-item" v-for="item in funcList" :key="item.op" :style="getitemStyle">
                    <a href="/WenZhang" :style="getAStyle" @click="clickOrder(item.op)"> {{item.pageName}}</a>
                </div>
            </li>
        </div>
      </div>
...
export default {
    ...
    data(){
        return {
            funcList: this.g.uiInfo.searchMap,
        };
    },
    methods:{
